...
class tuSimple(data.Dataset): 
    """ 
    image_set split into 3 partitions: 'train', 'val' and 'test'
    ...
    def __init__(self, path, image_set, transforms=None): 
        ...
        def createIndex(self): 
            ...
            If FileNotFoundError is raised, ensure that seg_label is not created in the directory.
            Deleting the directory and instantiating tuSimple class again will generate the labels
            """
            self.img_list = []
            self.segLabel_list = []
            self.exist_list = []

            listfile = os.path.join(self.data_pth, "seg_label", "list", "{}_gt.txt".format(self.image_set))
        ...
        def generate_label(self): 
            save_dir = os.path.join(self.data_path, "seg_label") 
            os.makedirs(save_dir, exist_ok=True)

            # -------- Merge label jsons into one file for train, val and test sets --------
            # -------- train set --------
            with open(os.path.join(save_dir, "train.json"), "w") as outfile: 
                for json_name in self.TRAIN_SET: 
                    with open(os.path.join(self.data_path, json_name)) as infile: 
                        for line in infile:
                            outfile.write(line) 
            # -------- val set --------
            ...
            print('train set labels generated in {}'.format(os.path.join(save_dir, "test.json")))
            self._gen_label_for_json('val')
            print('val set labels generated in {}'.format(os.path.join(save_dir, "test.json")))
            self._gen_label_for_json('test') 
            print('test set labels generated in {}'.format(os.path.join(save_dir, "test.json")))
        ...
        def _gen_label_for_json(self, image_set): 
            ...
            os.makedirs(os.path.join(self.data_path, save_dir, "list"), exist_ok=True)
            list_f = open(os.path.join(self.data_path, save_dir, "list", "{}_gt.txt".format(image_set)), "w")
            json_path = os.path.join(self.data_path, save_dir, "{}.json".format(image_set))
